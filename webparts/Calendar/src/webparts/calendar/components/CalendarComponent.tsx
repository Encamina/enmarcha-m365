...
> {
  ...
  private getHolidaysConfig = async (): Promise<void> => {
    ...
    const items: any[] = await sp.web.lists
      .getByTitle(this.props.configList)
      .items.select("Title", "Color")();
  ...
  private getHolidays = async (): Promise<void> => {
    ...
    const items: any[] = await sp.web.lists
      ...
      .items.select(
        ...
        "Categoria",
        "Fecha",
        "CategoriaLookup/Title",
        "CategoriaLookup/Color",
      )
  ...
  private goToPreviousMonth = (): void => {
    this.setState({
      firstDayOfActiveMonth: this.state.firstDayOfActiveMonth.minus({
        month: 1,
      }),
    });
  };
  ...
  private goToNextMonth = (): void => {
    this.setState({
      firstDayOfActiveMonth: this.state.firstDayOfActiveMonth.plus({
        month: 1,
      }),
    });
  };
        ...
        <div className={styles.calendar}>
          ...
          <div className={styles.calendar_weeks_grid}>
            {weekDays.map((weekDay, weekDayIndex) => (
              <div
                key={weekDayIndex}
                className={styles.calendar_weeks_grid_cell}
              >
                {weekDay}
              </div>
            ))}
            ...
            {daysOfMonth.map((dayOfMonth, dayOfMonthIndex) => (
              <div
                ...
                style={
                  ...
                  this.state.holidays.some((x) =>
                    dayOfMonth?.hasSame(x.fecha, "day"),
                  )
                    ? {
                        backgroundColor: this.state.holidays.filter((x) =>
                          dayOfMonth?.hasSame(x.fecha, "day"),
                        )[0].color,
                        color: "#ffffff",
                      }
